...
const StaffPage: React.FC = () => {
  ...
  useEffect(() => {
    if (currentMember) {
      setFormState({
        ...
        // normaliser types
        id_magasin:
          currentMember.id_magasin !== undefined && currentMember.id_magasin !== null
            ? Number(currentMember.id_magasin)
            : null,
        salaire_base:
          currentMember.salaire_base !== undefined && currentMember.salaire_base !== null
            ? Number(currentMember.salaire_base)
            : 0,
        date_embauche: currentMember.date_embauche ?? null,
      });
    } else {
      setFormState({
        nom: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // payload propre (éviter undefined pour les champs requis)
    const payload: Partial<StaffMember> = {
      ...
      poste: formState.poste || 'vendeur',
      statut: formState.statut || 'actif',
      matricule: (formState.matricule || '').trim(),
      salaire_base: typeof formState.salaire_base === 'number' ? formState.salaire_base : 0,
      date_embauche: formState.date_embauche || null,
      id_magasin:
        formState.id_magasin === '' || formState.id_magasin === undefined
          ? null
          : (formState.id_magasin as number | null),
      photo_url: formState.photo_url || userAvatarDataUrl,
    };

    try {
      if (currentMember?.id) {
        await apiClient.updateStaff(currentMember.id, payload);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await apiClient.deleteStaff(memberToDelete.id);
      setStaff((prev) => prev.filter((m) => m.id !== memberToDelete.id));
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    } else if (name === 'date_embauche') {
      next = value === '' ? null : value; // ISO string (yyyy-mm-dd)
    }
  ...
  return (
    <div className="p-4">
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50">
              <tr className="border-b border-gray-200">
                ...
                <th className="py-3 px-4 font-medium text-gray-500">Prénom</th>
                ...
                staff.map((member) => (
                  <tr key={member.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td className="py-2 px-4">
                      <img
                        src={member.photo_url || userAvatarDataUrl}
                        alt="Profil"
                    ...
                    <td className="py-3 px-4 text-gray-600">
                      {stores.find((s) => s.id === (member.id_magasin ?? -1))?.nom_magasin || 'Non affecté'}
                    </td>
                    <td className="py-3 px-4 text-gray-600">{member.telephone}</td>
                    <td className="py-3 px-4">
                      <span
                        className={`px-2 py-1 text-xs font-semibold rounded-full ${
                          member.statut === 'actif' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                        }`}
                      >
                        {member.statut}
                      </span>
                    ...
                    <td className="py-3 px-4 text-right">
                      <TableActions onEdit={() => handleOpenEditModal(member)} onDelete={() => handleOpenDeleteModal(member)} />
                    </td>
                  </tr>
                ))
              ) : (
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModals} title={currentMember?.id ? 'Modifier Personnel' : 'Nouveau Personnel'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 uppercase">Matricule</label>
              <input
                type="text"
                name="matricule"
            ...
            <div>
              ...
              <input
                type="number"
                name="salaire_base"
                value={formState.salaire_base ?? 0}
                onChange={handleChange}
                className="w-full mt-1 p-2 border rounded-md outline-none"
              />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-600 uppercase">Date d’embauche</label>
              <input
                type="date"
                name="date_embauche"
                value={formState.date_embauche ? formState.date_embauche.substring(0, 10) : ''}
                onChange={handleChange}
                className="w-full mt-1 p-2 border rounded-md outline-none"
              />
            </div>
          ...
          <div className="flex justify-end space-x-3 pt-4">
            <button type="button" onClick={handleCloseModals} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-md">
              Annuler
            </button>
            <button
              type="submit"
